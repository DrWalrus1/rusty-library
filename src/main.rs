...
async fn bind_queue_to_exchange(
    ...
    let (queue, _, _) = channel.queue_declare(queue_params).await.unwrap().unwrap();

    //check if the channel is open, if not open it
    if !channel.is_open() {
...
async fn system_info(connection_details: RabbitConnect) {
    // create a unique queue and bind it to the exchange system_monitor_queue
    // let uuid = Uuid::new_v4();
    let queue = "system_monitor_queue";
    ...
    //you can also put all connect logic in its own function and call it during setup and when the tokio join returns a Err option this would be a clean approach
    ...
    loop {
        ...
        bind_queue_to_exchange(&mut connection, &mut channel, &connection_details, "TestDirect", queue).await;
        let (ctag, mut messages_rx) = channel.basic_consume_rx(args.clone()).await.unwrap();

        let mut i = 0;
        while let Some(msg) = messages_rx.recv().await {
            let a = msg.content.unwrap();
            let basic_properties = msg.basic_properties.unwrap();

            let payload = String::from_utf8_lossy(&a);

            //call your own function and do something useful and return Ok or Err and on Ok ack the message, this way you don't loose messages
            //this is assuming there are no semantic errors in the message in that case when the message needs to be discarded also call ack.
            //but that is up to your functional error handling
            println!("Message Type {}, content {}", basic_properties, payload);
        ...
        // this is what to do when we get a error
...
async fn main() {
    ...
    //do note that we copy the connect_details instead of moving and borrowing you have to copy the details anyway when you have more than one task.
